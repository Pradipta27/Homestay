...
def proses_booking():
    ...
    cur4 = db.cursor()
    cur4.execute("SELECT guestId FROM guest")
    for row in cur4:
        guestfinal = (max(row))

    cur2 = db.cursor()
    cur2.execute('insert into booking (guestId, roomId, dateFrom, dateTo, total_price) values (%s, %s, %s, %s, %s)',
                 (guestfinal, roomId, checkIn, checkOut, bayar))
    db.commit()
